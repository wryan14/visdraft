...
class Visualization(db.Model):
    ...
    data_source = db.Column(db.Text)  # JSON string containing dataset id and column mapping
    ...
    def to_dict(self):
        return {
            ...
            'data_source': json.loads(self.data_source) if self.data_source else None,
            'shared': self.shared,
            'notes': self.notes
        }
...
class Dataset(db.Model):
    ...
    uploaded_at = db.Column(db.DateTime, default=datetime.utcnow)  # Added this field
...
def init_db(app):
    ...
    with app.app_context():
        ...
        # You could add some default templates here if none exist
        if Template.query.count() == 0:
            default_templates = [
                Template(
                    name="Monthly Circulation Trends",
                    description="Track circulation numbers over time",
                    chart_type="line",
                    ...
                    config=json.dumps({
                        ...
                        "layout": {
                            ...
                            "xaxis": {"title": "Month"},
                            "yaxis": {"title": "Items Circulated"}
                        }
                    })
                ),
                Template(
                    name="Collection Distribution",
                    description="View distribution of items across collections",
                    chart_type="bar",
            ...
            try:
                db.session.commit()
                logger.info("Added default templates")
            except Exception as e:
                logger.error(f"Error adding default templates: {str(e)}")
                db.session.rollback()