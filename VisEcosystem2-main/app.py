...
def create_app(config_file=None):
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = f'sqlite:///{os.path.join(basedir, "instance", "library_viz.db")}'
    ...
    def list_visualizations():
        """List all visualizations organized by category"""
        try:
            # Get all non-template visualizations
            visualizations = Visualization.query.filter_by(is_template=False).all()
            ...
            for viz in visualizations:
                category = viz.category or 'Uncategorized'
                categories.add(category)
                if category not in visualizations_by_category:
                    visualizations_by_category[category] = []
                visualizations_by_category[category].append(viz.to_dict())
            ...
            return render_template(
                'visualizations.html',
                visualizations_by_category=visualizations_by_category,
    ...
    @app.route('/api/viz/<int:viz_id>/update', methods=['POST'])
    def update_visualization(viz_id):
        ...
        try:
            ...
            viz.description = data.get('description', viz.description)
            viz.category = data.get('category', viz.category)
            viz.chart_type = data.get('chart_type', viz.chart_type)
            
            # Update configuration
            if 'config' in data:
                viz.config = json.dumps(data['config'])
            ...
            logger.error(f"Error updating visualization {viz_id}: {str(e)}")
            db.session.rollback()
            return jsonify({
                'success': False,
                'error': str(e)
    ...
    @app.route('/api/viz/<int:viz_id>/delete', methods=['DELETE'])
    def delete_visualization(viz_id):
        ...
        try:
            ...
            logger.error(f"Error deleting visualization {viz_id}: {str(e)}")
            db.session.rollback()
            ...
            response.headers['Content-Type'] = 'application/json'
            return response, 500
    @app.route('/api/dataset/<int:dataset_id>/delete', methods=['DELETE'])
        ...
        try:
            ...
            if dataset.file_path and os.path.exists(dataset.file_path):
                try:
                    ...
                    logger.error(f"Error deleting file {dataset.file_path}: {str(e)}")
