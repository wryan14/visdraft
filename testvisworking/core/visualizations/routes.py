...
@viz_bp.route('/get/<viz_name>', methods=['GET'])
...
@viz_bp.route('/delete/<viz_name>', methods=['POST'])
def delete_viz(viz_name):
...
@viz_bp.route('/duplicate/<viz_name>', methods=['POST'])
def duplicate_viz(viz_name):
    """Duplicate an existing visualization"""
    try:
        # Load original
        orig_path = os.path.join(SAVED_VIZ_DIR, f'{viz_name}.json')
        if not os.path.exists(orig_path):
            return jsonify({'error': 'Original visualization not found'}), 404
            
        with open(orig_path, 'r') as f:
            viz_data = json.load(f)
            
        # Create new name
        new_name = f"{viz_name}_copy"
        counter = 1
        while os.path.exists(os.path.join(SAVED_VIZ_DIR, f'{new_name}.json')):
            new_name = f"{viz_name}_copy_{counter}"
            counter += 1
            
        # Save duplicate with new name
        viz_data['vizName'] = new_name
        viz_data['vizDescription'] = f"Copy of {viz_name}"
        viz_data['created_at'] = datetime.utcnow().isoformat()
        
        with open(os.path.join(SAVED_VIZ_DIR, f'{new_name}.json'), 'w') as f:
...
@viz_bp.route('/')
def index():
    """Show list of saved visualizations"""
    try:
        os.makedirs(SAVED_VIZ_DIR, exist_ok=True)
        visualizations = []
        
        for filename in os.listdir(SAVED_VIZ_DIR):
            if filename.endswith('.json'):
                with open(os.path.join(SAVED_VIZ_DIR, filename), 'r') as f:
                    viz_data = json.load(f)
                    visualizations.append({
                        'vizName': viz_data.get('vizName', 'Untitled'),
                        'vizDescription': viz_data.get('vizDescription', ''),
                        'filename': viz_data.get('filename', ''),
                        'chartType': viz_data.get('chartType', 'Unknown'),
                        'created_at': viz_data.get('created_at', ''),
                        'updated_at': viz_data.get('updated_at', '')
                    })
                    
        # Sort by created_at desc
        visualizations.sort(key=lambda x: x['created_at'], reverse=True)
        
        return render_template('visualizations/index.html', visualizations=visualizations)
    except Exception as e:
        flash(f'Error loading visualizations: {str(e)}', 'error')
        return render_template('visualizations/index.html', visualizations=[])
